...
class LazyImage: NSObject {
    ...
    class private func lazyLoadImage(imageView:UIImageView, url:String?, isUserInteractionEnabled:Bool, completion: () -> Void) -> Void {
        ...
        //Lazy load image (Asychronous call)
        let l = url?.stringByReplacingOccurrencesOfString(" ", withString: "%20")
        let urlObject:NSURL = NSURL(string:l!)!
            ...
            let task = session.dataTaskWithRequest(urlRequest) {(data, response, error) in
                ...
                if response != nil {
                    ...
                    if httpResponse.statusCode != 200 {
                        Swift.debugPrint("LazyImage status code : \(httpResponse.statusCode)", terminator: "")
                    }
                }
                
                if data == nil {
                    if error != nil {
                        Swift.debugPrint("Error : \(error!.localizedDescription)", terminator: "")
                    }
                    Swift.debugPrint("LazyImage: No image data available", terminator: "")
                    return
                }
                ...
                dispatch_async(dispatch_get_main_queue(), { () -> Void in
                    ...
                    if let img = image {
                        ...
                        do {
                            try UIImagePNGRepresentation(img)!.writeToFile(imagePath, options: NSDataWritingOptions.DataWritingFileProtectionComplete)
    ...
    class func zoomImageView(imageView:UIImageView) -> Void {
        ...
        UIView.animateWithDuration(0.3, animations: {
            ...
            completion: {(value: Bool) in
                ...
                NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(LazyImage.rotated), name: UIDeviceOrientationDidChangeNotification, object: nil)
        })
    }
